...
FFI_PLUGIN_EXPORT void YUV420ToRGB(const unsigned char* y,
    ...
    for (size_t i = 0; i < width; ++i)
        for (size_t j = 0; j < height; ++j) {
            ...
            const size_t y_index = j * y_row_bytes + i;
            const size_t rgb_index = j * width + i;

            const int yp = y[y_index];
            const int up = u[uv_index];
            const int vp = v[uv_index];

            rgb[rgb_index * 3 + 0] = clip(yp + (1436 * vp >> 10) - 179);
            rgb[rgb_index * 3 + 1] = clip(yp - ((46549 * up + 93604 * vp) >> 17) + 135);
            rgb[rgb_index * 3 + 2] = clip(yp + (1814 * up >> 10) - 227);
        }
}
